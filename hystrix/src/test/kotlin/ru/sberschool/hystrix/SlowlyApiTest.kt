...
class SlowlyApiTest {
    ...
    private val clientPokemon = HystrixFeign.builder()
        .client(ApacheHttpClient())
        .decoder(JacksonDecoder(mapper))
        // для удобства тестирования задаем таймауты на 1 секунду
        .options(Request.Options(1, TimeUnit.SECONDS, 1, TimeUnit.SECONDS, true))
    ...
    fun testFallback() {
        ...
        MockServerClient("127.0.0.1", 18080)
            .`when`(
                ...
                HttpRequest.request()
                    .withMethod("GET")
                    .withPath("/item-category/stat-boosts")
            )
            .respond(
        ...
        // expect
        assertEquals("fallback", client.getItemCategory().name)
    }

    @Test
    fun testClient() {
        assertEquals("stat-boosts", clientPokemon.getItemCategory().name)
    }
}
